...
func findTags(ediUrl string) (tg []string) {
	if ediUrl == "" {
		return
	}
	// resp, err := http.Get(pl.EditorialUrl)
	...
	// client := &http.Client{Timeout: 10 * time.Second}
	req, err := http.NewRequest("GET", ediUrl, nil)
	if err != nil {
		return
	}
...
func updateJson() {
	out := "gstore.json"
	f, err := os.OpenFile(out, os.O_RDWR, 0666)
	if err != nil {
		panic(err)
	}
	defer f.Close()
