...
typedef struct SNode {
	char exit;
	int freq;
	struct SNode* child[256];  // why 256 ? bcz 256 bytes...
	Route* route;
} SNode;
...
void insert(SNode* root, const char* s, int n, Route* r) {
	SNode* cur = root;
	cur->freq++;
	for (int i = 0; i < n; i++) {
		int ch = s[i];
...
Route* find_route(SNode* root, const char* s, int n) {
	SNode* cur = root;
	for (int i = 0; i < n; i++) {
		int ch = s[i];
		if (!cur->child[ch]) {
...
bool register_route(char* method, char* path, void* (*func)(void* arg)) {
	Route* r = (Route*) malloc(sizeof(Route));
	if (r == NULL || route_storage == NULL) {
		return false;
	}
    r->method = method;
    r->path = path;
    r->func = func;
	insert(route_storage, path, strlen(path), r);
	return true;
}
...
void handle_client(void* arg) {
	...
	char* response = (char*) route->func(NULL);
    n = send(c->fd, response, sizeof(char) * strlen(response), 0);
...
void* not_found_404(void* arg) {
	return (void*) "404 not found\n";
}

void* hall_of_flex_page(void* arg) {
	return (void*) "Hall Of Flex!!\n";
}

void* home_page(void* arg) {
	return (void*) "Welcome";
}

void route_registration() {
    register_route("GET", "/", home_page);
	register_route("GET", "/flex", hall_of_flex_page);
}
...
bool init_server() {
	...
	if (bind(server_fd, (struct sockaddr*)&addr, sizeof(addr)) < 0 ) { perror("bind"); return false; };	// bind to port 127.0.0.1:8080
	if (listen(server_fd, 10) < 0) { perror("listen"); return false; }; // 10 backlog queue, why use backlog queue ? maximum number of pending connections waiting to be accepted
...
int main() {
	...
	while (!shutdown_signal) {
		int c_fd = accept(server_fd, NULL, NULL);	// blocks until a new client connects, until shutdown signal is sent.
		if (c_fd < 0) {
			continue;
		}
